...
(function (root) {
  ...
  function panelAssign() {
    ...
    var rows = state.roster.assignments.map(function (a) {
      ...
      var link = base + "experiments/" + a.Experiment + "/" + (String(a.Mode) === "assessed" ? "?exam=1" : "");
      return "<tr><td>" + esc(a.Batch) + "</td><td>" + esc(exp ? exp.title : a.Experiment) + "</td>" +
        "<td>" + esc(a.Mode) + "</td><td>" + esc(fmtDate(a.Due)) + "</td>" +
    ...
    return "<h2>Assignments</h2>" +
      "<p>Set which experiment a batch is doing and in which mode. The assessed link switches off the on-screen identification aids; the practice link leaves them on.</p>" +
      '<div class="tablewrap"><table><caption>Current assignments</caption>' +
      "<thead><tr><th>Batch</th><th>Experiment</th><th>Mode</th><th>Due</th><th>Link</th></tr></thead>" +
      "<tbody>" + (rows || '<tr class="pending"><td colspan="5">None yet.</td></tr>') + "</tbody></table></div>" +
  ...
  document.addEventListener("click", function (ev) {
    ...
    if (a === "saveAssign") {
      post("saveAssignment", {
        batch: val("aBatch"), experiment: val("aExperiment"), mode: val("aMode"),
